...
def main():
    ...
    for i in range(k):
        ...
        result = mean_cnn2d_method(train_name, test_name, valid_name, i)
    ...
    print('mean_cnn2d_method : ', all_result_value)

    print('ave : test_acc, num_hit, test_precision, test_recall, test_f1, durTime')
    print('mean_cnn2d_method : ', np.mean(all_result_value, axis=0))
    print('mean_cnn2d_method std: ', np.std(all_result_value, axis=0))

    print('Finished.')

def mean_cnn2d_method(train_name, test_name, valid_name, round_index):
    print('*** Run Mean 2d CNN ... (single axis mean)')

    model = Sequential()
    model.add(Conv2D(filters=16, kernel_size=(3, 3), input_shape=(DIMX, DIMY, 1)))
    model.add(LeakyReLU())
    ...
    # checkpoint
    bestModelSavePath = '/data/kuang/hcp_clj/para/mean_cnn2d_%s_weights_best_500.hdf5' % str(round_index)
    ...
    # generator 3
    def generator_MeanCNN2D_MandD(subject_name0, batch_size, train_valid_test_flag):
        ...
        if train_valid_test_flag in ['Train']:
            while True:
                ...
                for itr in range(itr_time):
                    ...
                    data_x_up = np.mean(data_x, axis=3).reshape(dnum, dx, dy, 1)
                    data_y = to_categorical(np.array(data_y), num_classes=numClass)
                    yield (data_x_up, data_y)
                    del data_x, data_y, data_x_up
                    gc.collect()
                gc.collect()
            ...
            one_data = np.mean(one_data, axis=2).reshape(dx, dy, 1)
            print('Show data info: ', one_data.shape)
            del datafile, one_data
            gc.collect()
            return

    generator_MeanCNN2D_MandD(train_name, 0, 'Info')
    ...
    startTime = time.time()
    model.fit_generator(generator=generator_MeanCNN2D_MandD(train_name, batch_size, 'Train'),
                        steps_per_epoch=stepTrain,
                        epochs=60, verbose=1, callbacks=[early_stop, checkpoint, lrate, time_callback],
                        validation_data=generator_MeanCNN2D_MandD(valid_name, batch_size, 'Valid'),
                        validation_steps=stepValid)
    endTime = time.time()
    durTime = endTime - startTime
    print('&&& MeanCNN2D Time: ', durTime)
    ...
    predNoneFlag = True
    print('Start Predict.')
    for test_item in generator_MeanCNN2D_MandD(test_name, batch_size, 'Test'):
    ...
    print('mean_cnn2d time (singleAxisMean): ', durTime)
    K.clear_session()
    # clear memory
