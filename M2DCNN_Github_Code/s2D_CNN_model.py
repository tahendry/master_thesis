...
def main():
    ...
    for i in range(k):
        ...
        result = mc_cnn2d_method(train_name, test_name, valid_name, 'z', i)
    ...
    print('mc_cnn2d_method : ', all_result_value)

    print('ave : test_acc, num_hit, test_precision, test_recall, test_f1, durTime')
    print('mc_cnn2d_method : ', np.mean(all_result_value, axis=0))
    print('mc_cnn2d_method std: ', np.std(all_result_value, axis=0))

    print('Finished.')

def mc_cnn2d_method(train_name, test_name, valid_name, channel_direction, round_index):
    print('*** Run MC CNN 2D (singel axis channel cnn) ...')

    inputShape = (DIMX, DIMY, DIMZ)
    if channel_direction == 'x':
        inputShape = (DIMY, DIMZ, DIMX)
    elif channel_direction == 'y':
        inputShape = (DIMX, DIMZ, DIMY)

    model = Sequential()
    model.add(Conv2D(filters=16, kernel_size=(3, 3), input_shape=inputShape, kernel_initializer=initializers.glorot_normal()))
    model.add(LeakyReLU())
    model.add(MaxPool2D(pool_size=(2, 2)))
    model.add(BatchNormalization())
    model.add(Conv2D(filters=32, kernel_size=(3, 3), kernel_initializer=initializers.glorot_normal()))
    model.add(LeakyReLU())
    model.add(MaxPool2D(pool_size=(2, 2)))
    model.add(BatchNormalization())
    model.add(Flatten())
    ...
    # checkpoint
    bestModelSavePath = '/data/kuang/hcp_clj/para/mccnn2d_%s_%s_weights_best.hdf5' % (channel_direction, str(round_index))
    ...
    # generator
    def generator_MCCNN2D_MandD(subject_name0, batch_size, train_valid_test_flag):
        ...
        if train_valid_test_flag in ['Train']:
            while True:
                ...
                for itr in range(itr_time):
                    ...
                    if channel_direction == 'x':
                        ...
                        data_x = np.transpose(data_x, (0, 1, 3, 2))
            ...
            if channel_direction == 'x':
                ...
                one_data = np.transpose(one_data, (0, 2, 1))
            print('Show data info: ', one_data.shape)
            del datafile, one_data
            gc.collect()
            return

    generator_MCCNN2D_MandD(train_name, 0, 'Info')
    ...
    startTime = time.time()
    model.fit_generator(generator=generator_MCCNN2D_MandD(train_name, batch_size, 'Train'),
                        steps_per_epoch=stepTrain,
                        epochs=60, verbose=1, callbacks=[early_stop, checkpoint, lrate, time_callback],
                        validation_data=generator_MCCNN2D_MandD(valid_name, batch_size, 'Valid'),
                        validation_steps=stepValid)
    endTime = time.time()
    durTime = endTime - startTime
    print('&&& MC CNN 2D Time: ', durTime)
    ...
    predNoneFlag = True
    print('Start Predict.')
    for test_item in generator_MCCNN2D_MandD(test_name, batch_size, 'Test'):
    ...
    print('mc_cnn2d_method time: ', durTime)
    K.clear_session()
    # clear memory
