...
def main():
    ...
    for i in range(k):
        ...
        result = pmc_cnn2d_method(train_name, test_name, valid_name, i)
    ...
    print('pmc_cnn2d_method : ', all_result_value)

    print('ave : test_acc, num_hit, test_precision, test_recall, test_f1, durTime')
    print('pmc_cnn2d_method : ', np.mean(all_result_value, axis=0))
    print('pmc_cnn2d_method std: ', np.std(all_result_value, axis=0))

    print('Finished.')

def pmc_cnn2d_method(train_name, test_name, valid_name, round_index):
    print('*** Run M2D CNN (multi_axis_channel_cnn) ...')

    up_model = Sequential()
    up_model.add(Conv2D(filters=16, kernel_size=(3, 3), input_shape=(DIMX, DIMY, DIMZ), kernel_initializer=initializers.glorot_normal()))
    up_model.add(LeakyReLU())
    up_model.add(MaxPool2D(pool_size=(2, 2)))
    up_model.add(BatchNormalization())
    up_model.add(Conv2D(filters=32, kernel_size=(3, 3), kernel_initializer=initializers.glorot_normal()))
    up_model.add(LeakyReLU())
    up_model.add(MaxPool2D(pool_size=(2, 2)))
    up_model.add(BatchNormalization())
    up_model.add(Flatten())

    front_model = Sequential()
    front_model.add(Conv2D(filters=16, kernel_size=(3, 3), input_shape=(DIMX, DIMZ, DIMY), kernel_initializer=initializers.glorot_normal()))
    front_model.add(LeakyReLU())
    front_model.add(MaxPool2D(pool_size=(2, 2)))
    front_model.add(BatchNormalization())
    front_model.add(Conv2D(filters=32, kernel_size=(3, 3), kernel_initializer=initializers.glorot_normal()))
    front_model.add(LeakyReLU())
    front_model.add(MaxPool2D(pool_size=(2, 2)))
    front_model.add(BatchNormalization())
    front_model.add(Flatten())

    left_model = Sequential()
    left_model.add(Conv2D(filters=16, kernel_size=(3, 3), input_shape=(DIMY, DIMZ, DIMX), kernel_initializer=initializers.glorot_normal()))
    left_model.add(LeakyReLU())
    left_model.add(MaxPool2D(pool_size=(2, 2)))
    left_model.add(BatchNormalization())
    left_model.add(Conv2D(filters=32, kernel_size=(3, 3), kernel_initializer=initializers.glorot_normal()))
    left_model.add(LeakyReLU())
    left_model.add(MaxPool2D(pool_size=(2, 2)))
    left_model.add(BatchNormalization())
    left_model.add(Flatten())

    up_input = Input(shape=(DIMX, DIMY, DIMZ))
    front_input = Input(shape=(DIMX, DIMZ, DIMY))
    left_input = Input(shape=(DIMY, DIMZ, DIMX))

    up_output = up_model(up_input)
    front_output = front_model(front_input)
    left_output = left_model(left_input)

    merged = concatenate([up_output, front_output, left_output], axis=-1)
    ...
    model = Model(inputs=[up_input, front_input, left_input], outputs=prediction)
    print(model.summary())
    ...
    # checkpoint
    bestModelSavePath = '/data/kuang/hcp_clj/para/pmccnn2d_%s_weights_best.hdf5' % str(round_index)
    ...
    #generator 3
    def generator_PMCCNN2D_MandD(subject_name0, batch_size, train_valid_test_flag):
        ...
        if train_valid_test_flag in ['Train']:
            while True:
                ...
                for itr in range(itr_time):
                    ...
                    data_x_left = np.transpose(data_x, (0, 2, 3, 1))
                    data_x_front = np.transpose(data_x, (0, 1, 3, 2))
                    data_x_up = data_x
                    final_data_x = [data_x_up, data_x_front, data_x_left]
                    data_y = to_categorical(np.array(data_y), num_classes=numClass)
                    yield (final_data_x, data_y)
                    del data_x, data_y, data_x_up, data_x_front, data_x_left, final_data_x
                    gc.collect()
                gc.collect()
            ...
            data_x_left = np.transpose(one_data, (1, 2, 0))
            data_x_front = np.transpose(one_data, (0, 2, 1))
            data_x_up = one_data
            print('Show data info: ', one_data.shape, data_x_up.shape, data_x_front.shape, data_x_left.shape)
            del datafile, one_data, data_x_up, data_x_front, data_x_left
            gc.collect()
            return

    generator_PMCCNN2D_MandD(train_name, 0, 'Info')
    ...
    startTime = time.time()
    model.fit_generator(generator=generator_PMCCNN2D_MandD(train_name, batch_size, 'Train'),
                        steps_per_epoch=stepTrain,
                        epochs=60, verbose=1, callbacks=[early_stop, checkpoint, lrate, time_callback, history],
                        validation_data=generator_PMCCNN2D_MandD(valid_name, batch_size, 'Valid'),
                        validation_steps=stepValid)
    endTime = time.time()
    durTime = endTime - startTime
    print('&&& PMCCNN2D Time: ', durTime)
    ...
    loss_file = '/home/kuang/project/hcp_clj/result/pmc_cnn2d_500_%s_loss.txt' % str(round_index)
    ...
    predNoneFlag = True
    print('Start Predict.')
    for test_item in generator_PMCCNN2D_MandD(test_name, batch_size, 'Test'):
    ...
    print('pmc_cnn2d_method time : ', durTime)
    K.clear_session()
    # clear memory
