...
def main():
    ...
    for i in range(k):
        ...
        result = cnn1d_method(train_name, test_name, valid_name)
    ...
    print('cnn1d_method : ', all_result_value)

    print('ave : test_acc, num_hit, test_precision, test_recall, test_f1, durTime')
    print('cnn1d_method : ', np.mean(all_result_value, axis=0))
    print('cnn1d_method std: ', np.std(all_result_value, axis=0))

    print('Finished.')

# CNN 1D ----------------------------------------------
def cnn1d_method(train_name, test_name, valid_name):
    print('*** Run 1d CNN ... ')
    ...
    model = Sequential()
    model.add(Conv1D(filters=16, kernel_size=3, input_shape=(steps, input_dim), kernel_initializer=initializers.glorot_normal()))
    model.add(LeakyReLU())
    model.add(MaxPool1D(pool_size=2))
    model.add(BatchNormalization())
    model.add(Conv1D(filters=32, kernel_size=3, kernel_initializer=initializers.glorot_normal()))
    model.add(LeakyReLU())
    model.add(MaxPool1D(pool_size=2))
    model.add(BatchNormalization())
    model.add(Flatten())
    ...
    # checkpoint
    bestModelSavePath = '/data/kuang/hcp_clj/para/cnn1d_weights_best.hdf5'
    ...
    # generator
    def generator_CNN1D_MandD(subject_name0, batch_size, train_valid_test_flag):
        ...
        if train_valid_test_flag in ['Train']:
            while True:
                ...
                for itr in range(itr_time):
                    ...
                    dx, dy, dz = data_x[0].shape
                    fdata_x = np.array([xx.reshape(dx, -1).transpose(1, 0) for xx in data_x])
                    data_y = to_categorical(np.array(data_y), num_classes=numClass)
                    yield (fdata_x, data_y)
                    del data_x, data_y, fdata_x
                    gc.collect()
                gc.collect()
            ...
            one_data = one_data.reshape(dx, -1).transpose(1, 0)
            print('Show data info: ', one_data.shape)
            del datafile, one_data
            gc.collect()
            return

    generator_CNN1D_MandD(train_name, 0, 'Info')
    ...
    startTime = time.time()
    model.fit_generator(generator=generator_CNN1D_MandD(train_name, batch_size, 'Train'),
                        steps_per_epoch=stepTrain,
                        epochs=60, verbose=1, callbacks=[early_stop, checkpoint, lrate, time_callback],
                        validation_data=generator_CNN1D_MandD(valid_name, batch_size, 'Valid'),
                        validation_steps=stepValid)
    endTime = time.time()
    durTime = endTime - startTime
    print('&&& CNN 1D Time: ', durTime)
    ...
    predNoneFlag = True
    print('Start Predict.')
    for test_item in generator_CNN1D_MandD(test_name, batch_size, 'Test'):
